...
public class TargetAgent : Agent
{
    ...
    {
        // If the other object is a catcher, negative reward and end episode
        if (other.tag == "Player")
        {
            AddReward(-1f);
            EndEpisode();
        ...
        if (other.tag == "button" && allowButton == 0)
        {
            //reset position of the gate
            /*gate.transform.position = new Vector3(-10.5f, 0.5f, -5f);
            gate.transform.rotation = Quaternion.Euler(Vector3.zero);
            ...
            transform.position = new Vector3(-12f, 0.5f, -6f);
